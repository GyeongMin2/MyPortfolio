                ...
                ${boardDTO.boardContent}
                    ...
                    <c:if test="${not empty fileList}">
                        <c:forEach var="file" items="${fileList}">
                            <!-- URL 디코딩 추가 -->
                            <c:set var="decodedPath" value="${fn:replace(file.filePath, '%2F', '/')}"/>
                            
                            <c:if test="${file.fileName.toLowerCase().endsWith('.jpg') || 
                                         file.fileName.toLowerCase().endsWith('.png') || 
                                         file.fileName.toLowerCase().endsWith('.jpeg') ||
                                         file.fileName.toLowerCase().endsWith('.gif')}">
                                <img src="${pageContext.request.contextPath}${decodedPath}" alt="${file.fileName}" style="width: 500px; height: auto;">
                            </c:if>
                            <a href="${pageContext.request.contextPath}${decodedPath}">${file.fileName}</a>
                        </c:forEach>
                    </c:if>
              <div class="navMainBoundary"></div>
